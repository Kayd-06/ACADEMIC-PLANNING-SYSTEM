...
const CATEGORY_META: Record<string, { icon: React.ElementType; color: string }> = {
  Announcement: { icon: Megaphone, color: 'text-indigo-500 bg-indigo-50' },
  Result: { icon: FileBarChart, color: 'text-emerald-500 bg-emerald-50' },
  Assignment: { icon: ClipboardList, color: 'text-amber-500 bg-amber-50' },
  Fee: { icon: CreditCard, color: 'text-rose-500 bg-rose-50' },
  Attendance: { icon: CalendarCheck, color: 'text-sky-500 bg-sky-50' },
...
function NotificationBell() {
  ...
  const visible = categoryFilter === 'All' ? items : items.filter(n => n.category === categoryFilter)
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute right-0 top-11 w-[380px] bg-white border border-slate-200 rounded-2xl shadow-2xl z-[100] overflow-hidden flex flex-col max-h-[520px]"
          >
            <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 shrink-0">
              <div className="flex items-center gap-2">
                <Bell className="w-4 h-4 text-slate-600" />
                <h3 className="text-sm font-bold text-slate-900">Notifications</h3>
                {unreadCount > 0 && <span className="text-[10px] font-bold text-rose-600 bg-rose-50 px-1.5 py-0.5 rounded-full">{unreadCount} new</span>}
              </div>
              {unreadCount > 0 && (
                <button onClick={markAllRead} disabled={loading} className="flex items-center gap-1 text-[11px] font-semibold text-indigo-600 hover:text-indigo-800 disabled:opacity-50">
            ...
            {categories.length > 2 && (
              <div className="flex gap-1.5 px-4 py-2 border-b border-slate-100 overflow-x-auto shrink-0">
                {categories.map(c => (
                  <button key={c} onClick={() => setCategoryFilter(c)}
                    className={`px-2.5 py-1 rounded-full text-[10px] font-bold whitespace-nowrap transition-colors ${categoryFilter === c ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
                    {c}
                  </button>
                ))}
              </div>
                ...
                visible.map(n => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-start gap-3 px-4 py-3 border-b border-slate-50 text-left transition-colors ${n.isRead ? 'bg-white hover:bg-slate-50/50' : 'bg-indigo-50/40 hover:bg-indigo-50/70'}`}
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2">
                          <p className={`text-[12px] leading-snug ${n.isRead ? 'font-medium text-slate-600' : 'font-bold text-slate-900'}`}>{n.title}</p>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider">{n.category}</span>
