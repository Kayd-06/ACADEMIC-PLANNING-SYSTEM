...
export function MultiSelectTargetExam({
  value,
  onChange,
  className = ''
}: {
  ...
  const handleToggle = (exam: string) => {
    let next: string[]
    if (selected.includes(exam)) {
      next = selected.filter((e) => e !== exam)
    } else {
      next = [...selected, exam]
    }
    onChange(next.join(', '))
  ...
  return (
    <div className={`relative ${className}`} ref={containerRef}>
      ...
      >
        {selected.length === 0 ? (
          <span className="text-slate-400 my-0.5">Select Target Exam(s)...</span>
        ) : (
          <div className="flex flex-wrap gap-1">
            {selected.map((exam) => (
              <span
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            ...
            {/* Custom exams already added that aren't in TARGET_EXAMS */}
            {selected.filter(e => !TARGET_EXAMS.includes(e)).length > 0 && (
              <>
                <div className="border-t border-slate-100 my-1" />
                <div className="px-3 py-1 text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                  Custom Exams
                ...
                {selected
                  .filter((e) => !TARGET_EXAMS.includes(e))
                  .map((exam) => (
                    <button
                      key={exam}
                      type="button"
