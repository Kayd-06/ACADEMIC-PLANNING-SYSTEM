                      ...
                      <tr key={item.id} className="hover:bg-slate-50/20 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center justify-center gap-1.5">
                            <button
                              onClick={() => { setUploadTarget(item); setUploadFile(null); setUploadError('') }}
                              title="Upload file"
                              className="p-1.5 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 border border-slate-100 bg-white rounded-lg transition-colors inline-flex items-center justify-center shadow-sm"
                            >
                              <Paperclip className="w-4 h-4" />
                            </button>
                            {item.fileUrl && (
                              <button
                                onClick={() => setPreviewAssignment(item)}
                                title="Preview uploaded file"
                                className="p-1.5 text-slate-400 hover:text-violet-600 hover:bg-violet-50 border border-slate-100 bg-white rounded-lg transition-colors inline-flex items-center justify-center shadow-sm"
                              >
                                <FileText className="w-4 h-4" />
                              </button>
                            )}
                            <button
                              onClick={() => { setSelectedAssignment(item); setShowDetailsModal(true) }}
                              title="View details"
          ...
          <div className="fixed inset-0 z-[999] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
            ...
            >
              <div className="flex justify-between items-center mb-5">
                <div>
                  <h3 className="text-base font-bold text-slate-900">Upload Assignment File</h3>
                  <p className="text-xs text-slate-500 mt-0.5 truncate max-w-[280px]">{uploadTarget.title}</p>
                </div>
                <button onClick={() => setUploadTarget(null)} className="text-slate-400 hover:text-slate-600 p-1 rounded-lg hover:bg-slate-50">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="space-y-4">
                {uploadTarget.fileUrl && (
                  <div className="flex items-center gap-2 px-3 py-2 bg-emerald-50 border border-emerald-100 rounded-xl text-xs font-semibold text-emerald-700">
                    <CheckCircle className="w-4 h-4 shrink-0" />
                    File already uploaded — uploading again will replace it.
                  </div>
                )}

                <label className="flex flex-col items-center justify-center gap-3 border-2 border-dashed border-slate-200 rounded-xl p-8 cursor-pointer hover:border-indigo-400 hover:bg-indigo-50/30 transition-colors">
                  <Paperclip className="w-7 h-7 text-slate-400" />
                  <div className="text-center">
                    <p className="text-xs font-bold text-slate-700">{uploadFile ? uploadFile.name : 'Click to choose a file'}</p>
                    <p className="text-[11px] text-slate-400 mt-0.5">PDF, images, Word, or any document</p>
                  </div>
                  <input type="file" className="hidden" onChange={e => { setUploadFile(e.target.files?.[0] || null); setUploadError('') }} />
                </label>
              ...
              <div className="flex items-center gap-3 pt-5 border-t border-slate-100 mt-5">
                <button onClick={() => setUploadTarget(null)} className="flex-1 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-xl text-xs transition-colors">
                  Cancel
                </button>
                <button
        ...
        {previewAssignment && (
          <div className="fixed inset-0 z-[999] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0, scale: 0.95 }}
              className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl border border-slate-100 flex flex-col"
              style={{ height: '85vh' }}
            >
              <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 shrink-0">
                <div>
                  <h3 className="text-sm font-bold text-slate-900">{previewAssignment.title}</h3>
                  <p className="text-xs text-slate-500 mt-0.5">{previewAssignment.chapter}</p>
                </div>
                <div className="flex items-center gap-2">
                  <a href={previewAssignment.fileUrl} target="_blank" rel="noopener noreferrer"
                  ...
                  <button onClick={() => setPreviewAssignment(null)} className="text-slate-400 hover:text-slate-600 p-1.5 rounded-lg hover:bg-slate-50">
                    <X className="w-5 h-5" />
                  </button>
                </div>
              </div>
              <div className="flex-1 overflow-hidden rounded-b-2xl">
                {/\.(jpg|jpeg|png|gif|webp|svg)$/i.test(previewAssignment.fileUrl) ? (
                  <div className="h-full flex items-center justify-center bg-slate-50 p-4">
                    <img src={previewAssignment.fileUrl} alt="Assignment file" className="max-h-full max-w-full object-contain rounded-lg" />
                  </div>
                ) : (
                  <iframe src={previewAssignment.fileUrl} className="w-full h-full border-0" title="Assignment file preview" />
                )}
              </div>
            </motion.div>
          </div>
