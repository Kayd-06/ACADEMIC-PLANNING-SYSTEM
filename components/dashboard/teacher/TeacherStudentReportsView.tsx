    ...
    try {
      ...
      reader.onload = async (e) => {
        try {
          ...
          // Expecting headers: Name, RollNo, Marks, MaxMarks
          ...
          if (res.ok) {
            ...
            showToast('Upload Failed', 'error', 'Server rejected the report data.')
          }
        } catch (err) {
          console.error(err)
                ...
                {reports.map((rep) => (
                  <tr key={rep._id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right">
                      <button onClick={() => showToast('View Details', 'info', 'Opening report breakdown...')} className="text-[12px] font-semibold text-indigo-600 hover:underline">View Details</button>
                    </td>
                  </tr>
                ))}
              </tbody>
                    ...
                    {file ? (
                      ...
                      <>
                        ...
                        <p className="text-[11px] text-slate-500 mb-4">
                          Requires columns: Name, RollNo, Marks, MaxMarks
                        </p>
                      </>
                    )}
                    
                    <button onClick={(e) => { e.stopPropagation(); showToast('Download Sample', 'info', 'Downloading CSV template...') }} className="flex items-center gap-1.5 text-[12px] font-semibold text-indigo-600 hover:text-indigo-700 z-10 relative">
